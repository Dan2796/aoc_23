...
public class Day08Tests
{
    ...
    {
        Dictionary<long, int> fifteen = CamelMaps.CalcPrimeFactors(15);
        fifteen[3].Should().Be(1);
        fifteen[5].Should().Be(1);
        fifteen.Keys.Should().NotContain(1);
        Dictionary<long, int> oneHundred = CamelMaps.CalcPrimeFactors(100);
        oneHundred[2].Should().Be(2);
        oneHundred[5].Should().Be(2);
        oneHundred.Keys.Should().NotContain(3);
        Dictionary<long, int> two = CamelMaps.CalcPrimeFactors(2);
        two[2].Should().Be(1);
        Dictionary<long, int> three = CamelMaps.CalcPrimeFactors(3);
        three[3].Should().Be(1);
    }
    ...
    public void CamelMaps_CalcLCM_GetsLCMOf23()
    {
        ...
        long lcm = CamelMaps.CalcLCM(testMultiples);
        lcm.Should().Be(6);
    }
    [Fact] 
